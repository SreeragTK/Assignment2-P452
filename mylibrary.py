...
def inverse_calculator(a, method,x,tolerance,iterations):
    '''
   Here A= matrix of which the inverse is being calculated 
   method = which iterative method we are using for calculating the inverse
...
'''
def poly_fit(x,y,delta_y,order=1):
    X=[]
    Y=[]
    B=[]
	k=order+1 
    #It is the number of parameters
	x=np.array(x)
	y=np.array(y)
