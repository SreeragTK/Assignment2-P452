...
def pi_calculation(n):
    ...
    in_circle = []#points inside circle
    out_circle = []#points outside circle
    for i in range (n):
        p=[]
        p.append(pts1[i])
        p.append(pts2[i])
...
The volume of the region is
    ...
    integral integral V~ -2- integral 1 1- x V~ -----2 1 2 16- V = -1 - V~ 1-x2 2 1- x dydx = -14|\1 - x /|dx = 3 . 
'''
#Defining function for finding the volume of the given Steinmentz solid
#volume is the integral of (4*(1-x^2)) in the region 0,1
def f2(x):
    k=4*(1-x**2)
    return k
...
print("The corresponding error for the volume is", (100*(3/16)*(np.abs(16/3-volume))))
'''
Results are
...
The corresponding error for the volume is 0.30566793979872053
'''